...
### fare_transfer_rules.txt 
 ...
 1. Les groupes de segments tarifaires applicables définis dans `fare_leg_rules.txt` doivent être déterminés pour tous les segments individuels ou les segments tarifaires effectifs du voyage en fonction du trajet du passager. 
...
| `duration_limit_type` | Énumération |**Requis sous condition**| Définit le début et la fin relatifs de `fare_transfer_rules.duration_limit`.<br><br> Les options valides sont :<br> `0` - Entre la validation du tarif de départ du premier segment du sous-voyage de transfert et la validation du tarif d’arrivée du dernier segment du sous-voyage de transfert.<br> `1` - Entre la validation du tarif de départ du premier segment du sous-voyage de transfert et la validation du tarif de départ du dernier segment du sous-voyage de transfert.<br> `2` - Entre la validation du tarif d’arrivée du premier segment du sous-voyage de transfert et la validation du tarif de départ du dernier segment du sous-voyage de transfert.<br> `3` - Entre la validation du tarif d’arrivée du premier segment du sous-voyage de transfert et la validation du tarif d’arrivée du dernier segment du sous-voyage de transfert.<br><br> Lorsqu’une règle de transfert avec les mêmes `from_leg_group_id` et `to_leg_group_id` est appliquée plusieurs fois consécutivement au cours d’un voyage à plusieurs étapes, la `duration_limit` spécifiée par la règle doit être mesurée à partir de la première étape correspondante.<br><br> Requis sous condition:<br> -**Requis**si `fare_transfer_rules.duration_limit` est défini.<br> -**Interdit**si `fare_transfer_rules.duration_limit` est vide. |
 ...
 | `fare_product_id` | ID étranger faisant référence à `fare_products.fare_product_id` | Optionnel | Produit tarifaire requis pour le transfert entre deux segments tarifaires. Si vide, le coût de la règle de transfert est de 0.| 
