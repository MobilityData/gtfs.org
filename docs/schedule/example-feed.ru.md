...
Этот пример GTFS feed показывает примеры данных, разделенных запятыми, для каждого файла в транзитной ленте. Показанные здесь образцы файлов данных не все связаны друг с другом. Вы также можете [загрузить полный фид GTFS](https://github.com/google/transit/blob/master/gtfs/spec/en/examples/sample-feed-1.zip?raw=true) в окончательном виде для работы или просмотреть отдельные файлы этого же фида онлайн [здесь](https://github.com/google/transit/tree/master/gtfs/spec/en/examples/sample-feed-1).
...
`location_type=` 0: платформа; 1: станция; 2: вход/выход; 3: общий узел; 4: зона посадки.
...
Этот пример показывает исключения в обслуживании для праздника Дня независимости в 2006 году. В понедельник, 3 июля 2006 года, регулярное обслуживание в будние дни`(service_id=WD`) прерывается`(exception_type=2`). Вместо этого в этот день работает сервис выходного дня`(service_id=WE`)`(exception_type=1`). Это же изменение действует и во вторник 4 июля.
