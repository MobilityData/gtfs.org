...
이는 [일반 대중교통 피드 사양(GTFS)](../reference)에서 대중교통 서비스를 설명하기 위한 권장 사례입니다. 이러한 관행은 [GTFS 권장사항 실무 그룹](#gtfs-best-practices-working-group) 회원의 경험과 [애플리케이션별 GTFS 관행 권장사항](http://www.transitwiki.org/TransitWiki/index.php/Best_practices_for_creating_GTFS) 을 바탕으로 종합되었습니다.
...
* 서비스 수정 사항이 7일 이내에 적용되는 경우 [GTFS-realtime](https://developers.google.com/transit/gtfs-realtime/) 피드(서비스 권고 사항 또는 여행 업데이트)를 통해 이 서비스 변경 사항을 표현하세요. 대신 정적 GTFS 데이터세트를 사용합니다.
...
현재 모든 모범 사례를 준수하는지 확인하는 유효성 검사 도구는 없습니다. 다양한 유효성 검사 도구는 이러한 모범 사례 중 일부를 준수하는지 확인합니다. GTFS 검사기 도구 목록은 [GTFS 검사기](https://github.com/CUTR-at-USF/awesome-transit#gtfs-validators)를 참조하세요. 이러한 권장사항을 참조하는 GTFS 검사기 도구를 작성하는 경우 [specifications@mobilitydata.org](mailto:specifications@mobilitydata.org)로 이메일을 보내주세요.
...
[specifications@mobilitydata.org](mailto:specifications@mobilitydata.org)로 이메일을 보내거나 [GitHub GTFS Best Practices repo](https://github.com/rocky-mountain-institute/gtfs-best-practices)에서 문제 또는 풀 요청을 엽니다.
...
GTFS 애플리케이션과 관행은 발전하므로 이 문서는 수시로 수정되어야 할 수도 있습니다. 이 문서의 수정안을 제안하려면 [GTFS Best Practices GitHub repository](https://github.com/MobilityData/gtfs-best-practices) 에서 풀 요청을 열고 변경 사항을 옹호하세요. 의견이 있으시면 [specifications@mobilitydata.org](mailto:specifications@mobilitydata.org)로 이메일을 보내실 수 있습니다.
