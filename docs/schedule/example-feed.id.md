...
Contoh feed GTFS ini menampilkan sampel data yang dipisahkan koma untuk setiap file dalam feed transit. File data sampel yang ditampilkan di sini tidak semuanya terkait satu sama lain. Anda juga dapat [mendownload feed GTFS lengkap](https://github.com/google/transit/blob/master/gtfs/spec/en/examples/sample-feed-1.zip?raw=true) dalam bentuk final untuk digunakan, atau menjelajahi file satu per satu dari feed yang sama secara online [di sini](https://github.com/google/transit/tree/master/gtfs/spec/en/examples/sample-feed-1) .
...
`location_type=` 0: platform; 1: stasiun; 2: masuk/keluar; 3: simpul umum; 4: area keberangkatan.
...
Contoh ini menunjukkan pengecualian layanan untuk liburan Hari Kemerdekaan tahun 2006. Pada hari Senin tanggal 3 Juli 2006, layanan hari kerja biasa ( `service_id=WD` ) terganggu ( `exception_type=2` ). Sebaliknya, layanan akhir pekan ( `service_id=WE` ) berjalan pada tanggal tersebut ( `exception_type=1` ). Perubahan yang sama juga berlaku pada Selasa 4 Juli.
