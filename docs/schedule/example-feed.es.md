...
# Ejemplo de feed GTFS

Este ejemplo de alimentación GTFS muestra muestras de datos delimitados por comas para cada archivo en una alimentación de tránsito. Los archivos de datos de muestra que se muestran aquí no están todos relacionados entre sí. También puede [descargar un feed GTFS completo](https://github.com/google/transit/blob/master/gtfs/spec/en/examples/sample-feed-1.zip?raw=true) en su forma final para trabajar con él, o examinar los archivos individuales de ese mismo feed en línea [aquí](https://github.com/google/transit/tree/master/gtfs/spec/en/examples/sample-feed-1).
...
`location_type=` 0: andén; 1: estación; 2: entrada/salida; 3: nodo genérico; 4: zona de embarque.
...
Este ejemplo muestra las excepciones de servicio para el día de la Independencia de 2006. El lunes 3 de julio de 2006, el servicio regular de días laborables`(service_id=WD`) se interrumpe`(exception_type=2`). En su lugar, el servicio de fin de semana (`service_id=WE`) funciona en esa fecha (`exception_type=1`). El mismo cambio se aplica también al martes 4 de julio.
