...
| `cemv_support` | 列挙型 |任意| 乗客が、運賃検証システム（ペイ・アズ・ユー・ゴーやオープンループシステムなど）において、非接触型EMV（Europay、Mastercard、Visa）カードまたはモバイルデバイスを運賃認証チケットメディアとして使用することで、この交通事業者（乗車など）にアクセスできるかどうかを示します。このフィールドは、cEMVが他のチケット商品を購入したり、他の運賃チケットメディアに付加価値を付けたりするために使用できることを示すものではありません。<br><br> cEMV のサポートは、この事業者のすべてのサービスがチケットメディアとして cEMV カードまたはモバイル デバイスを使用してアクセスできる場合にのみ示されるするべきである。<br><br>有効なオプションは次のとおりです。<br><br> `0` または空 - この事業者に関連付けられた便の cEMV 情報はありません。<br> `1` - 乗客は、この事業者に関連する便のチケットメディアとして cEMV を使用できしてもよい。<br> `2` - この事業者に関連付けられた便のチケットメディアとして cEMV はサポートされていません。<br><br>同じサービスに対して ` agency.cemv_support` と `routes.cemv_support` の両方が指定されている場合は、 `routes.cemv_support` の値が優先されしなければならない。<br><br>このフィールドは他のすべての運賃関連ファイルとは独立しており、個別に使用してもよい。このフィールドと運賃関連ファイル（例：`fare_media.txt`、 `fare_products.txt`、 `fare_leg_rules.txt` ）の情報が矛盾する場合は、それらのファイルの情報が` agency.cemv_support`よりも優先されしなければならない。|
...
| `stop_access` | 列挙型 |**条件付きで禁止**| 特定の駅の停留所へのアクセス方法を示します。有効なオプションは次のとおりです。<br><br> `0` - 停留所/プラットフォームには道路網から直接アクセスできません。駅に入口が定義されている場合は駅入口から、そうでない場合は駅自体からアクセスするしなければならないがあります。駅に構内通路が定義されている場合は、それを使用して停留所/プラットフォームにアクセスするしなければならない。<br> `1` - 利用アプリケーションは、親駅の入口や構内通路に関係なく、停留所に直接アクセスするための道順を生成するするべきである。<br><br> `stop_access` が空の場合、指定された停止またはプラットフォームへのアクセスは未定義と見なされます。<br><br>**条件付きで禁止**:<br> - 駅 (`location_type=1`)、入口 (`location_type=2`)、汎用ノード (`location_type=3`)、または乗車エリア (`location_type=4`) の場所では**禁止**です。<br> - `parent_station`が空の場合は**禁止**です。<br> - それ以外の場合は任意。 |
...
| `cemv_support` | 列挙型 |任意| 乗客が、運賃検証装置（ペイ・アズ・ユー・ゴーやオープンループシステムなど）において、非接触型EMV（Europay、Mastercard、Visa）カードまたはモバイルデバイスを運賃チケットメディアとして使用することで、このルートに関連付けられた交通サービス（つまり、乗車）にアクセスできるかどうかを示します。このフィールドは、cEMVを使用して他のチケット商品を購入したり、他のチケットメディアに付加価値を付けたりできることを示すものではありません。<br><br> cEMV のサポートは、このルートのすべてのサービスがチケットメディアとして cEMV カードまたはモバイル デバイスを使用してアクセスできる場合にのみ示されるするべきである。<br><br>有効なオプションは次のとおりです。<br><br> `0` または空 - このルートに関連付けられた便の cEMV 情報はありません。<br> `1` - 乗客は、このルートに関連する便のチケットメディアとして cEMV を使用できしてもよい。<br> `2` - このルートに関連付けられた便では、cEMV はチケットメディアとしてサポートされていません。<br><br>同じサービスに対して ` agency.cemv_support` と `routes.cemv_support` の両方が指定されている場合は、 `routes.cemv_support` の値が優先されしなければならない。<br><br>このフィールドは、他のすべての運賃関連ファイルから独立しており、個別に使用してもよい。このフィールドと運賃関連ファイル (`fare_media.txt`、 `fare_products.txt`、 `fare_leg_rules.txt`など) との間に矛盾する情報がある場合、それらのファイルの情報は `agency.cemv_support` よりも優先されしなければならない。|
...
| `cars_allowed` | 列挙型 |任意| 車が許可されているかどうかを示します。有効なオプションは次のとおりです。<br><br> `0` または空 - 旅行の車両情報がありません。<br> `1` - この特定の旅行で使用される車両には、少なくとも 1 台の車を収容できます。<br> `2` - このルートでは車の乗り入れは禁止されています。|
...
構内通路は、駅の内部アクセス グラフを網羅的に定義することを目的としています。駅内に構内通路が定義されている場合、データ コンシューマーは、その駅内のすべての関連する接続が記述されていると想定するするべきである。ただし、 `stops.txt`の任意の`stop_access` フィールドを使用すると、停留所が街路ネットワークから直接アクセスできるか、駅の定義済み構内通路を介してアクセスできるかを明示的に定義してもよい。したがって、次のガイドラインが適用されます:

- ぶら下がった場所なし: 駅構内のいずれかの場所に通路がある場合は、その駅内のすべての場所に構内通路がするべきである。ただし、
    - 乗車エリアがあるプラットフォーム ( `location_type=4`、以下のガイドラインを参照)
    - ` 停留所 .stop_access=1` の停留所( `location_type=0`または空)
...
- ロックされたプラットフォームなし: 駅構内のいずれかの場所に通路がある場合は、各プラットフォーム ( `location_type=0`または空) または乗車エリア (`location_type=4` ) は、少なくとも 1 つの入口/出口 ( `location_type=2`) に何らかの構内通路のチェーンを介して接続されているしなければならない。ただし、次の場合は除きます:
    - 停留所 ( `location_type=0`または空) が `停留所.stop_access=1` で明示的にマークされている場合、この場合は道路網から直接アクセスできる必要があります。
...
| `from_stop_id` | `stops.stop_id` を参照する外部 ID | **必須** |経路が始まる場所です。<br><br>プラットフォーム (`location_type=0` または空)、入口/出口 (`location_type=2`)、汎用ノード (`location_type=3`)、または乗車エリア (`location_type=4`) を識別する `stop_id` を含める必要があります。<br><br> 駅 ( `location_type=1`)、または `stop_access=1` の停留所(`location_type=0`または空) を識別する`stop_id`の値は使用できません。|
| `to_stop_id` | `stops.stop_id` を参照する外部 ID | **必須** |経路が終了する場所です。<br><br>プラットフォーム (`location_type=0` または空)、入口/出口 (`location_type=2`)、汎用ノード (`location_type=3`)、または乗車エリア (`location_type=4`) を識別する `stop_id` を含める必要があります。<br><br> 駅 ( `location_type=1`)、または `stop_access=1` の停留所(`location_type=0`または空) を識別する`stop_id`の値は使用できません。|
