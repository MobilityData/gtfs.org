...
これらは、[General Transit Feed Specific (GTFS)](../reference) で公共交通事業者のサービスを記述するための推奨プラクティスです。これらのプラクティスは、[GTFS ベスト プラクティス ワーキング グループ](#gtfs_5) メンバーの経験と [アプリケーション固有の GTFS プラクティス推奨事項](http://www.transitwiki.org/TransitWiki/index.php/Best_practices_for_creating_GTFS) からまとめられたものです。

詳細な背景情報については、[よくある質問](#faq) を参照してください。

## ドキュメント構造

プラクティスは、4 つの主要なセクションに分かれています。

* __[データセットの公開と一般的なプラクティス](#_2):__ これらのプラクティスは、GTFS データセットの全体的な構造と、GTFS データセットが公開される方法に関連しています。
* __[ファイル別に整理された実践推奨事項](#_3):__ 実践を公式 GTFS リファレンスにマッピングしやすくするために、推奨事項は GTFS 内のファイルとフィールド別に整理されています。
* __[ケース別に整理された実践推奨事項](#_5):__ ループ ルートなどの特定のケースでは、実践を複数のファイルとフィールドに適用する必要がある場合があります。このような推奨事項はこのセクションにまとめられています。
...
| フィールド名 | 推奨事項 |
|---|---|
| `agency_phone` | そのようなカスタマー サービス電話番号が存在する限り、含める必要があります。 |
| `agency_email` | そのようなカスタマー サービス メールが存在する限り、含める必要があります。 |
...
| | ルート グループに明確に名前が付けられたブランチ (例: 1A と 1B) が含まれている場合は、ルート [分岐](#_7) ケースの推奨事項に従って、`route_short_name` と `route_long_name` を決定します。 |
| `route_color` と `route_text_color` | 標識、印刷された顧客情報、オンラインの顧客情報と一致している必要があります (他の場所に存在しない場合は含まれません)。 |

### trips.txt 

* __環状ルート・路線系統の特殊なケースを参照してください :__ 環状ルート・路線系統は、2 つの異なる終点がある線形ルート・路線系統とは対照的に、便が同じ停留所で開始および終了する場合です。環状ルート・路線系統は、特定のプラクティスに従って記述するしなければならない。[ループ ルートのケースを参照してください。](#_6)
...
環状ルート・路線系統: 環状ルート・路線系統では、特別な`stop_times`の考慮が必要です。(参照: [ループ ルートのケース](#_6))
...
`feed_info.txt` に [proposed feed\_contact\_email or feed\_contact\_url](https://github.com/google/transit/pull/31/files) フィールドがある場合はそれを使用して、または交通事業者またはフィード プロデューサーのウェブサイトで連絡先情報を検索して、フィードの連絡先を特定します。フィード プロデューサーに問題を伝えるときは、議論中の特定の GTFS ベスト プラクティスにリンクします。 （[`このドキュメントへのリンク`](#_14)を参照してください）。
