...
Каждый [StopTimeUpdate](../reference.md#message-stoptimeupdate) связан с остановкой. Обычно это можно сделать, используя либо GTFS stop_sequence, либо GTFS stop_id. Однако, в случае, если вы предоставляете обновление для поездки без `trip_id` GTFS, вы должны указать stop_id, так как stop_sequence не имеет значения. stop_id все равно должен ссылаться на stop_id в GTFS. Если один и тот же stop_id посещается более одного раза в поездке, то stop_sequence должен быть предоставлен во всех StopTimeUpdates для этого stop_id в этой поездке.
