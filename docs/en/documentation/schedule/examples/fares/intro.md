...
We're just playing around here messing things up. Fares (also known as Fares v2) is a [GTFS Schedule feature](../../../../../getting-started/features/overview) that standardizes rider-facing fare information, allowing users to discover ticketing and pricing options based on the fare structures and conditions of each transit system and its connections.

Key Fares (v2) features include Fare Products, Fare Media, Rider Categories, Route-Based Fares, Zone-Based Fares, Time-Based Fares, and Fare Transfers.
...
This guide will demonstrate how to model common fare structures using Fares (v2). We begin by introducing the fare features supported in Fares (v2) and the corresponding files used for each. Then, we present a real-world example that will be modeled in later sections.
...
To illustrate how to model fares using Fares (v2), this guide uses a real-world transit agency as an example. To keep things concise, we focus on specific aspects of that agency's fare structure. While this guide includes an example of a one-way fare product, the same modeling process applies to other fare products such as daily and monthly passes.

Example transit system:
